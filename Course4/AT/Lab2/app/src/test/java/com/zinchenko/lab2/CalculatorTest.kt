...
class CalculatorTest
{
	...
	fun multiOperations()
	{
		assertEquals("8.0", calc.process("8/2*4+4-12"))
	}

	@Test
	fun addMultiplyPriority()
	{
		assertEquals("6.0", calc.process("2+2*2"))
	}

	@Test
	fun subMultiplyPriority()
	{
		assertEquals("-2.0", calc.process("2-2*2"))
	}

	@Test
	fun addDividePriority()
	{
		assertEquals("12.0", calc.process("8+8/2"))
	}

	@Test
	fun subDividePriority()
	{
		assertEquals("4.0", calc.process("8-8/2"))
	}

	@Test
	fun addMultiplyBracketsPriority()
	{
		assertEquals("8.0", calc.process("(2+2)*2"))
	}

	@Test
	fun subMultiplyBracketsPriority()
	{
		assertEquals("0.0", calc.process("(2-2)*2"))
	}

	@Test
	fun addDivideBracketsPriority()
	{
		assertEquals("6.0", calc.process("(8+4)/2"))
	}

	@Test
	fun subDivideBracketsPriority()
	{
		assertEquals("2.0", calc.process("(8-4)/2"))
	}
