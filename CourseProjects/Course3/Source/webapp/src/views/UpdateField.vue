<template>
	<div>
		<vk-card padding=large class="uk-width-extend@m">			
			<vk-card-title>{{ $t('field_edit') }}</vk-card-title>
...
export default
{
	...
	mounted ()
	{
		...
		const current = Store.getters.FIELDS.find(item => item.id === id);
		this.id = id;
		this.agriculture = current.agriculture.id;
		this.agricultureText = current.agriculture.name;
	...
	methods:
	{
		...
		async addField()
		{
			await Store.dispatch('UPD_FIELD',
			{
				id: this.id,
